...
public class CreateUserValidatorTests
{
    ...
    public void Setup()
    {
        ...
        _validator = new CreateUserRequestValidator(_userRepositoryMock.Object, new PasswordValidator());
    }
    ...
    {
        var createUserRequest = new CreateUserRequest(new CreateUserDTO { FullName = "Test", Email = "testemail@gmail.com", Password = "pass" });
        var result = await _validator.TestValidateAsync(createUserRequest);
        result.ShouldHaveValidationErrorFor(x => x.CreateUserDto.Password)
            .WithErrorMessage(Constants.Validation.User.PasswordMinLengthMessage);
        Assert.That(result.Errors.Count, Is.EqualTo(1));
    }
}
