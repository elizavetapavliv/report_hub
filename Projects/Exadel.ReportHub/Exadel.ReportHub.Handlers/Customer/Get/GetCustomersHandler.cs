...
public record GetCustomersRequest(bool? IsDeleted) : IRequest<ErrorOr<IEnumerable<CustomerDTO>>>;
...
public class GetCustomersHandler(ICustomerRepository customerRepository, IMapper mapper) : IRequestHandler<GetCustomersRequest, ErrorOr<IEnumerable<CustomerDTO>>>
{
    ...
    {
        var customers = await customerRepository.GetAsync(request.IsDeleted, cancellationToken);
